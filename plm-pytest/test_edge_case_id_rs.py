...
def test_rs_plink_PML_T34(reset_state, srcRS, dstRS, plink, id_type):
        ...
        for meta in collections_meta:
            ...
            for _ in range(num_batches):
                ...
                for _ in range(batch_size):
                    _id = (
                        random.uniform(1e5, 1e10) if id_type == "float" else Decimal128(str(random.uniform(1e5, 1e10)))
                    )
                    doc = {
                        "_id": _id,
                        "payload": "x" * 5000,
    ...
    expected_mismatches = [("stress_test_db", "hash mismatch"), ("stress_test_db.regular_coll", "hash mismatch")]
