...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ["repl_test_db.*"],
            ["repl_test_db.multi_key_indexes"],
            [("repl_test_db", "hash mismatch")],
            ["init_test_db", "clone_test_db", "repl_test_db.multi_key_indexes"],
        ),
        ([], ["init_test_db.compound_indexes"], [("init_test_db", "hash mismatch")], ["init_test_db.compound_indexes"]),
    ],
)
def test_rs_plink_PML_T35(
    reset_state, srcRS, dstRS, plink, include_namespaces, exclude_namespaces, skip_entries, skip_prefixes
):
    """
    ...
    filtered_mismatches = [
        ...
        if not (any(name.startswith(prefix) for prefix in skip_prefixes) or (name, reason) in skip_entries)
    ]
...
def test_rs_plink_PML_T36(reset_state, srcRS, dstRS, plink):
    ...
    try:
        _, operation_threads_1 = create_all_types_db(srcRS.connection, "init_test_db", start_crud=True)
        result = plink.start(
            include_namespaces=["init_test_db.*", "repl_test_db.*"],
