...
def create_collection_types(db, create_ts=False, drop_before_creation=False):
    ...
    french_docs = [{"string": "cote"}, {"string": "coté"}, {"string": "côte"}, {"string": "côté"}]
    ...
    if create_ts:
        ...
        db.create_collection(
            "timeseries_data", timeseries={"timeField": "timestamp", "metaField": "metadata", "granularity": "seconds"}
        )
...
def perform_crud_ops_collection(collection, capped=False, timeseries=False):
    ...
    new_doc = {"_id": ObjectId(), "string": "New Inserted String", "int": 99, "boolean": False}
    ...
    collection.replace_one({"string": "Updated String"}, {"string": "Completely Replaced", "int": 5})

    collection.delete_one({"string": "Batch Insert 3"})
    collection.delete_many({"int": {"$gt": 50}})

    collection.update_one(
        {"string": "Upserted Document"}, {"$set": {"string": "Upserted Document", "int": 150}}, upsert=True
    )

    collection.update_one(
        {"string": "Upserted Document"}, {"$set": {"string": "Upserted Document New", "int": 5}}, upsert=True
    )

    collection.update_many({"category": "missing"}, {"$set": {"category": "default"}}, upsert=True)

    collection.find_one_and_update(
        {"string": "FindOneAndUpdate Upsert"}, {"$set": {"string": "FindOneAndUpdate Upserted"}}, upsert=True
    )

    collection.find_one_and_update(
        {"string": "FindOneAndUpdate Upserted"}, {"$set": {"string": "FindOneAndUpdate Upserted New"}}, upsert=True
    )

    collection.find_one_and_replace(
        {"string": "FindOneAndReplace Upsert"}, {"string": "FindOneAndReplace Upserted"}, upsert=True
    )

    collection.find_one_and_replace(
        {"string": "FindOneAndReplace Upserted"}, {"string": "FindOneAndReplace Upserted New"}, upsert=True
    )
    ...
    bulk_operations = [
        ...
        pymongo.ReplaceOne(
            {"string": "FindOneAndReplace Upserted New"}, {"string": "FindOneAndReplace Upserted Bulk"}, upsert=True
        ),
    ]

    if not capped:
        bulk_operations += [pymongo.DeleteOne({"string": "Batch Insert 1"}), pymongo.DeleteMany({"int": {"$lt": 50}})]
