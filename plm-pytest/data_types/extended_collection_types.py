...
def create_diff_coll_types(db, drop_before_creation=False):
    ...
    # Simple view
    db.command({"create": "customer_view", "viewOn": "customers", "pipeline": [{"$match": {"status": "active"}}]})
            ...
            "pipeline": [
                {"$match": {"status": "active"}},
                {
                    "$project": {
                        "_id": 1,
                        "name": 1,
                        "age_group": {"$cond": [{"$gte": ["$age", 30]}, "Senior", "Junior"]},
                    }
                },
            ],
        }
    ...
    large_doc = {"_id": 1, "name": "Large Document Test", "data": "x" * 16777161}
