...
class Cluster:
    ...
    def connection(self):
        if self.layout == "replicaset":
            ...
            return f"mongodb://root:root@{hosts}/?replicaSet={self.config['_id']}"
        else:
            return "mongodb://root:root@" + self.config["mongos"] + ":27017/"

    @property
    def plink_connection(self):
        if self.layout == "replicaset":
            hosts = ",".join(f"{member['host']}:27017" for member in self.config["members"])
            return f"mongodb://root:root@{hosts}/?replicaSet={self.config['_id']}"
        else:
            return "mongodb://plink:test1234@" + self.config["mongos"] + ":27017/"
    ...
    def setup_authorization(host):
        ...
        init_s_user = (
            '\'db.getSiblingDB("admin").createUser({ user: "system", pwd: "system", roles: [ "__system" ] });\''
        )
        logs = primary.check_output("mongosh -u root -p root --quiet --eval " + init_s_user)
        Cluster.log("Adding plink user on " + host)
        plink_user = (
            '\'db.getSiblingDB("admin").createUser({user:"plink",pwd:"test1234","roles":['
        ...
        x509_plink_user = (
            '\'db.getSiblingDB("$external").runCommand({createUser:"emailAddress=plm@percona.com,CN=plm,OU=client,O=Percona,L=SanFrancisco,ST=California,C=US","roles":['
            + '{"db":"admin","role":"backup" },'
            + '{"db":"admin","role":"clusterMonitor" },'
            + '{"db":"admin","role":"clusterManager" },'
            + '{"db":"admin","role":"restore" },'
        ...
        logs = primary.check_output("mongosh -u root -p root --quiet --eval " + plink_user)
        logs = primary.check_output("mongosh -u root -p root --quiet --eval " + x509_plink_user)

    def __setup_authorizations(self, replicasets):
        with concurrent.futures.ProcessPoolExecutor() as executor:
            for rs in replicasets:
                executor.submit(Cluster.setup_authorization, rs["members"][0]["host"])
        time.sleep(1)
    ...
    def wait_for_primary(host, connection):
        ...
        while True:
            result = n.run("mongosh " + connection + " --quiet --eval 'db.hello().isWritablePrimary'")
