...
def test_data_integrity_check_PML_T1(start_cluster, srcRS, dstRS):
    ...
    for collection in expected_mismatches:
        assert collection in summary, f"Mismatch for collection {collection} isn't detected"
    ...
    src["test_db1"]["test_coll2"].create_index(
        [("key", pymongo.ASCENDING), ("data", pymongo.DESCENDING)], name="index_compound"
    )
    ...
    src["test_db1"]["test_coll4"].create_index(
        [("key", pymongo.ASCENDING)], name="index_partial", partialFilterExpression={"data": {"$gt": 5}}
    )
    ...
    src["test_db1"]["test_coll7"].create_index(
        [("createdAt", pymongo.ASCENDING)], name="index_ttl", expireAfterSeconds=3600
    )
    dst["test_db1"]["test_coll7"].create_index(
        [("createdAt", pymongo.ASCENDING)], name="index_ttl", expireAfterSeconds=7200
    )
    ...
    for index in expected_mismatches:
        assert index in summary, f"Mismatch for index {index} isn't detected"
    ...
    dst["test_db1"]["test_coll2"].create_index(
        [("key", pymongo.ASCENDING), ("data", pymongo.DESCENDING)], name="index_compound"
    )
    dst["test_db1"]["test_coll3"].create_index([("$**", pymongo.TEXT)], name="index_wildcard")
    dst["test_db1"]["test_coll4"].create_index(
        [("key", pymongo.ASCENDING)], name="index_partial", partialFilterExpression={"data": {"$gt": 5}}
    )
