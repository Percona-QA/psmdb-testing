...
@pytest.mark.parametrize("csync_env, listen_host_args, expect_start, expect_reachable, internal_check_host", [
    ({}, "", True, False, None),
    ({"PCSM_LISTEN_HOST": "0.0.0.0"}, "", True, True, None),
    ({}, "--listen-host=0.0.0.0", True, True, None),
    ({"PCSM_LISTEN_HOST": "::1"}, "", True, False, "[::1]"),
    ({"PCSM_LISTEN_HOST": "localhost"}, "", True, False, None),
    ({"PCSM_LISTEN_HOST": "localhost:2242"}, "", False, None, None),
    ({"PCSM_LISTEN_HOST": "127.0.0.1:2242"}, "", False, None, None),
    ({"PCSM_LISTEN_HOST": "[::1]:2242"}, "", False, None, None),
    ({}, "--listen-host=localhost:2242", False, None, None),
], indirect=["csync_env"])
def test_listen_host_PCSM_345(csync, src_cluster, dst_cluster, csync_env, listen_host_args, expect_start, expect_reachable, internal_check_host):
    """
