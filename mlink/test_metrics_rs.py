...
def assert_metrics(metrics):
    ...
    expected_metrics_with_checks = {
        ...
        'percona_mongolink_copy_insert_batch_duration_seconds': lambda v: 0 <= v <= 1,
        'percona_mongolink_copy_insert_document_total': lambda v: 0 <= v <= 100_000,
        'percona_mongolink_copy_insert_size_bytes_total': lambda v: 0 <= v <= 2**30,
        'percona_mongolink_copy_read_batch_duration_seconds': lambda v: 0 <= v <= 1,
        'percona_mongolink_copy_read_document_total': lambda v: 0 <= v <= 100_000,
        'percona_mongolink_copy_read_size_bytes_total': lambda v: 0 <= v <= 2**30,
        'percona_mongolink_estimated_total_size_bytes': lambda v: 0 <= v <= 2**30,
        'percona_mongolink_events_processed_total': lambda v: 0 <= v <= 50_000,
        'percona_mongolink_initial_sync_lag_time_seconds': lambda v: v >= 0 and v <= 600,
        'percona_mongolink_lag_time_seconds': lambda v: v >= 0 and v <= 600,
        'percona_mongolink_process_cpu_seconds_total': lambda v: 0 <= v <= 600,
        'percona_mongolink_process_max_fds': lambda v: 0 < v <= 2**31,
        'percona_mongolink_process_network_receive_bytes_total': lambda v: 0 <= v <= 2**30,
        'percona_mongolink_process_network_transmit_bytes_total': lambda v: 0 <= v <= 2**30,
        'percona_mongolink_process_open_fds': lambda v: 1 <= v <= 100,
        'percona_mongolink_process_resident_memory_bytes': lambda v: 0 < v <= 2*2**30,
        'percona_mongolink_process_start_time_seconds': lambda v: 1_700_000_000 <= v <= 2_000_000_000,
        'percona_mongolink_process_virtual_memory_bytes': lambda v: 0 < v <= 10*2**30,
        'percona_mongolink_process_virtual_memory_max_bytes': lambda v: 0 < v <= 2**64,
    }
...
def test_rs_mlink_PML_T44(reset_state, srcRS, dstRS, mlink):
    """
    Test to validate metrics returned by mlink service
    """
    try:
