...
def test_rs_mlink_PML_T50(reset_state, srcRS, dstRS, mlink):
    ...
    collection.insert_many([
        ...
        {"_id": 13, "arr": ["A", "B", "C"]},
        {"_id": 14, "nums": [1, 2, 3]}])
    assert mlink.start() and mlink.wait_for_repl_stage()
    ...
    assert mlink.wait_for_zero_lag() and mlink.finalize()
    time.sleep(1)
    ...
    assert mlink.check_mlink_errors()[0]
...
@pytest.mark.timeout(300, func_only=True)
@pytest.mark.usefixtures("start_cluster")
def test_rs_mlink_PML_T53(reset_state, srcRS, dstRS, mlink):
...
@pytest.mark.timeout(300, func_only=True)
@pytest.mark.usefixtures("start_cluster")
def test_rs_mlink_PML_T54(reset_state, srcRS, dstRS, mlink):
    ...
    try:
        ...
        pytest.xfail("Known limitation: PML-139")
    assert mlink.check_mlink_errors()[0]