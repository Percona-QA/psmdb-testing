...
primary_rs = testinfra.utils.ansible_runner.AnsibleRunner(os.environ["MOLECULE_INVENTORY_FILE"]).get_host("primary-rs")

secondary1_rs = testinfra.utils.ansible_runner.AnsibleRunner(os.environ["MOLECULE_INVENTORY_FILE"]).get_host(
    "secondary1-rs"
)

secondary2_rs = testinfra.utils.ansible_runner.AnsibleRunner(os.environ["MOLECULE_INVENTORY_FILE"]).get_host(
    "secondary2-rs"
)
...
def load_data(node, port, count):
    ...
    result = node.check_output(
        "mgodatagen --uri=mongodb://127.0.0.1:" + port + "/?replicaSet=rs -f /tmp/generated_config.json --batchsize 10"
    )


def check_count_data(node, port):
    result = node.check_output(
        "mongo mongodb://127.0.0.1:" + port + "/test?replicaSet=rs --eval 'db.binary.count()' --quiet | tail -1"
    )
...
def drop_database(node, port):
    result = node.check_output(
        "mongo mongodb://127.0.0.1:" + port + "/test?replicaSet=rs --eval 'db.dropDatabase()' --quiet"
    )
    print(result)
...
def test_1_setup_storage():
    result = primary_rs.check_output(
        "pbm config --mongodb-uri=mongodb://localhost:27017/ --file=/etc/pbm-agent-storage-"
        + STORAGE
        + ".conf --out=json"
    )
    store_out = json.loads(result)
...
def test_4_setup_pitr():
    ...
    if BACKUP_TYPE == "physical":
        result = primary_rs.check_output(
            "pbm config --mongodb-uri=mongodb://localhost:27017/ --set pitr.enabled=true --set pitr.oplogOnly=true --out=json"
        )
        for i in range(TIMEOUT):
        ...
        result = primary_rs.check_output(
            "pbm config --mongodb-uri=mongodb://localhost:27017/ --set pitr.enabled=true --out=json"
        )
    store_out = json.loads(result)
    print(store_out)
...
def test_7_disable_pitr():
    if EXISTING_BACKUP != "no" or CHECK_PITR == "no":
        pytest.skip("Skipping pitr test")
    result = primary_rs.check_output(
        "pbm config --mongodb-uri=mongodb://localhost:27017/ --set pitr.enabled=false --out=json"
    )
    store_out = json.loads(result)
    print(store_out)
    time.sleep(60)
