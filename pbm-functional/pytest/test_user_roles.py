...
def test_logical_PBM_T216(start_cluster, cluster, newcluster, restore_type):
    ...
    client_shard.administration.command(
        "createRole",
        "customAdministrationDBRoleSh",
        privileges=[
            {"resource": {"db": "administration", "collection": ""}, "actions": ["find", "insert", "update", "remove"]}
        ],
        roles=["readWrite"],
    ...
    client.admin.command(
        "createUser", "admin_random_user1", pwd="test123", roles=[{"role": "customAdminRole", "db": "admin"}]
    )
    client_shard.admin.command(
        "createUser", "admin_random_user2", pwd="test123", roles=[{"role": "customAdminRoleSh", "db": "admin"}]
    )
    client.test_db1.command(
        "createUser", "test_random_user1", pwd="test123", roles=[{"role": "customTestDBRole", "db": "test_db1"}]
    )
    client_shard.test_db1.command(
        "createUser", "test_random_user2", pwd="test123", roles=[{"role": "customTestDBRoleSh", "db": "test_db1"}]
    )
    client.administration.command(
        "createUser",
        "administration_random_user1",
        pwd="test123",
        roles=[{"role": "customAdministrationDBRole", "db": "administration"}],
    )
    client_shard.administration.command(
        "createUser",
        "administration_random_user2",
        pwd="test123",
    ...
    restore_commands = {
        ...
        "full_bck_part_rst_wo_user": " --base-snapshot="
        + backup_full
        + pitr
        + " --ns=administration.*,test_db1.*,test_db2.*",
        "full_bck_part_rst_user1": " --base-snapshot="
        + backup_full
        + pitr
        + " --ns=administration.*,test_db1.*,test_db2.* --with-users-and-roles",
        "full_bck_part_rst_user2": " --base-snapshot="
        + backup_full
        + pitr
        + " --ns=administration.* --with-users-and-roles",
    ...
    run_check(
        ...
        "admin",
        "admin_random_user3",
        {"readWrite", "userAdminAnyDatabase", "clusterAdmin"},
        restore_type in ["full_bck", "full_pitr"],
        restore_type,
    )
    run_check(
    ...
    run_check(
        ...
        "test_random_user1",
        {"customTestDBRole"},
        restore_type not in ["part_bck", "full_bck_part_rst_wo_user", "full_bck_part_rst_user2"],
        restore_type,
    )
    run_check(
        client_shard,
    ...
    run_check(
        ...
        "test_db1",
        "test_random_user3",
        {"readWrite", "clusterManager"},
        restore_type
        not in ["part_bck", "full_bck_part_rst_wo_user", "full_bck_part_rst_user1", "full_bck_part_rst_user2"],
        restore_type,
    )
    run_check(
        client_shard,
    ...
    run_check(
        ...
        "administration_random_user1",
        {"customAdministrationDBRole"},
        restore_type not in ["part_bck", "full_bck_part_rst_wo_user"],
        restore_type,
    )
    run_check(
        client_shard,
