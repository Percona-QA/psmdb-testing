...
def test_logical_timeseries_PBM_T224(start_cluster, cluster):
    ...
    pymongo.MongoClient(cluster.connection)["test"].create_collection(
        "test1", timeseries={"timeField": "timestamp", "metaField": "data"}
    )
    for i in range(10):
    ...
    pymongo.MongoClient(cluster.connection)["test"].create_collection(
        "test2", timeseries={"timeField": "timestamp", "metaField": "data"}
    )
    for i in range(10):
        pymongo.MongoClient(cluster.connection)["test"]["test1"].insert_one({"timestamp": datetime.now(), "data": i})
