...
def mongod_version():
    return (
        docker.from_env()
        .containers.run(image="replica_member/local", remove=True, command="mongod --version")
...
def test_physical_PBM_T195(start_cluster, cluster):
    ...
    for member in cluster.config["members"]:
        ...
        assert (
            "secondaryDelaySecs" not in member
            or member["secondaryDelaySecs"] == rs_config["members"][index]["secondaryDelaySecs"]
        )
