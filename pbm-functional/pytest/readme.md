...
```docker-compose up -d``` - create *test* network, start and configure storages for tests  - volume *fs* as fs-storage, container *seaweedfs* as AWS S3-like storage and container *azurite* as emulator for Azure Blob Storage.
...
- **cluster.setup_pbm()** - configure pbm with SeaweedFS S3 storage, the same as ```pbm config --file=/etc/pbm.conf --out=json```
