...
def test_physical_PBM_T299(start_cluster, docker_client):
    """
    ...
    backup = backup_cache.get("backup")
    if not backup:
        collection.insert_many(documents)
        backup = cluster.make_backup("physical")
        backup_cache["backup"] = backup
        assert collection.delete_many({}).deleted_count == len(documents), "Document deletion failed"
    else:
    ...
    if fallback_expected:
        expected_docs = pre_restore_docs
    else:
