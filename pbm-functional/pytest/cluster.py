...
class Cluster:
    ...
    def config(self, value):
        ...
        def validate_rs(rs):
            assert isinstance(rs["_id"], str) and isinstance(rs["members"], list)
        ...
        if set(value.keys()) == {"_id", "members"}:
            ...
            assert (
                isinstance(value["configserver"], dict)
                and isinstance(value["shards"], list)
                and isinstance(value["mongos"], str)
            )
            hosts = []
            ids = []
        ...
        if self.layout == "replicaset":
            for host in self.config["members"]:
                ...
                docker.from_env().containers.run(
                    ...
                    network="test",
                    environment=[
                        "AUTOSTART_CE=" + autostart_ce,
                        "AUTOSTART_PSMDB=" + autostart_psmdb,
                        "PBM_MONGODB_URI=" + pbm_mongodb_uri,
                        "DATADIR=" + self.mongod_datadir,
            ...
            for host in self.config["configserver"]["members"]:
                ...
                docker.from_env().containers.run(
                    ...
                    environment=[
                        ...
                        "KRB5_TRACE=/dev/stderr",
                        "MONGODB_EXTRA_ARGS= --port 27017 --replSet "
                        + self.config["configserver"]["_id"]
                        + " --configsvr --keyFile /etc/keyfile "
                        + mongod_args,
                        "GOCOVERDIR=/gocoverdir/reports",
                    ],
                    volumes=["fs:/backups", "keytabs:/keytabs", "gocoverdir:/gocoverdir"],
    ...
    def make_backup(self, type):
        ...
        while True:
            ...
            if not running:
                if type:
                    start = n.run("pbm backup --out=json --type=" + type)
                else:
    ...
    def make_restore(self, name, **kwargs):
        ...
        result = n.run(
            "SSL_CERT_FILE=/etc/nginx-minio/ca.crt timeout "
            + str(timeout)
            + " pbm restore "
            + name
            + " "
        ...
        if "--fallback-enabled=true" in restore_opts:
            ...
            for host in self.mongod_hosts:
                try:
                    container = docker.from_env().containers.get(host)
                    get_logs = container.exec_run("cat /var/lib/mongo/pbm.restore.log", stderr=False)
                    if get_logs.exit_code == 0:
                        Cluster.log("!!!!Possible failure on {}, file pbm.restore.log was found:".format(host))
    ...
    def disable_pitr(self, time_param=None):
        ...
        if time_param:
            ...
            while pitr_end < target_time:
                ...
                if (
                    "backups" in backups
                    and "pitrChunks" in backups["backups"]
                    and "pitrChunks" in backups["backups"]["pitrChunks"]
                ):
        ...
        result = n.check_output("pbm config --set pitr.enabled=false --wait --out json")
    ...
    def setup_replicaset(replicaset):
        ...
        init_rs = "'config =" + json.dumps(rs) + ";rs.initiate(config);'"
    ...
    def setup_authorization(host, uri):
        ...
        init_pbm_user = (
            '\'db.getSiblingDB("admin").createUser({user:"pbm",pwd:"pbmpass","roles":['
        ...
        init_pbm_t_user = (
            '\'db.getSiblingDB("admin").createUser({user:"pbm_test",pwd:"pbmpass_test1","roles":['
            + '{"db":"admin","role":"readWrite","collection":""},'
            + '{"db":"admin","role":"backup" },'
            + '{"db":"admin","role":"clusterMonitor" },'
            + '{"db":"admin","role":"restore" },'
        ...
        x509_pbm_user = (
            '\'db.getSiblingDB("$external").runCommand({createUser:"emailAddress=pbm@percona.com,CN=pbm,OU=client,O=Percona,L=SanFrancisco,ST=California,C=US","roles":['
            + '{"db":"admin","role":"readWrite","collection":""},'
            + '{"db":"admin","role":"backup" },'
            + '{"db":"admin","role":"clusterMonitor" },'
            + '{"db":"admin","role":"restore" },'
        ...
        krb_pbm_user = (
            '\'db.getSiblingDB("$external").runCommand({createUser:"pbm@PERCONATEST.COM","roles":['
            + '{"db":"admin","role":"readWrite","collection":""},'
            + '{"db":"admin","role":"backup" },'
            + '{"db":"admin","role":"clusterMonitor" },'
            + '{"db":"admin","role":"restore" },'
        ...
        ldap_mongo_grp = (
            '\'db.getSiblingDB("admin").runCommand({createRole:"cn=readers,ou=groups,dc=example,dc=org",privileges: [],"roles":['
            + '{"db":"admin","role":"readWrite","collection":""},'
            + '{"db":"admin","role":"backup" },'
            + '{"db":"admin","role":"clusterMonitor" },'
            + '{"db":"admin","role":"restore" },'
        ...
        logs = primary.check_output("mongo -u root -p root --quiet --eval " + init_pbm_t_user)
        # Cluster.log(logs)
        if "authMechanism=MONGODB-X509" in uri:
            logs = primary.check_output("mongo -u root -p root --quiet --eval " + x509_pbm_user)
            # Cluster.log(logs)
        if "authMechanism=GSSAPI" in uri:
            logs = primary.check_output("mongo -u root -p root --quiet --eval " + krb_pbm_user)
            # Cluster.log(logs)
        if "authMechanism=PLAIN" in uri:
            logs = primary.check_output("mongo -u root -p root --quiet --eval " + ldap_mongo_grp)
            # Cluster.log(logs)

    def __setup_authorizations(self, replicasets):
        with concurrent.futures.ProcessPoolExecutor() as executor:
            for rs in replicasets:
                executor.submit(Cluster.setup_authorization, rs["members"][0]["host"], self.pbm_mongodb_uri)
        time.sleep(1)
    ...
    def wait_for_primary(host, connection):
        ...
        while True:
            result = n.run("mongo " + connection + " --quiet --eval 'db.hello().isWritablePrimary'")
    ...
    def wait_for_primaries(self):
        ...
        with concurrent.futures.ProcessPoolExecutor() as executor:
            for primary in self.primary_hosts:
                executor.submit(Cluster.wait_for_primary, primary, "mongodb://root:root@127.0.0.1:27017")
    ...
    def check_initsync_single(host):
        n = testinfra.get_host("docker://" + host)
        result = n.check_output(
            "mongo -u root -p root --quiet --eval \"db.adminCommand( { getLog:'global'} ).log.forEach(x => {print(x)})\" | grep INITSYNC"
        )
    ...
    def external_restore_start(self):
        ...
        while True:
            ...
            if time.time() > timeout:
                ...
                logs = n.check_output("pbm logs -sD -t0")
                assert False, (
                    "Cannot start restore, another operation running: "
                    + str(self.get_status()["running"])
                    + "\n"
                    + logs
                )
            time.sleep(1)
            ...
            for shard in self.config["shards"]:
                ...
                for node in shard["members"]:
                    ...
                    if node["host"] not in self.arbiter_hosts:
                        n.check_output("cp -rp " + files + " /var/lib/mongo/")
                        n.check_output(
                            "touch /var/lib/mongo/pbm.restore.log && chown mongodb /var/lib/mongo/pbm.restore.log"
                        )
                        Cluster.log("Copying files " + files + " to host " + node["host"])
        else:
