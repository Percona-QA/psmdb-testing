...
class Cluster:
    ...
    def config(self,value):
        ...
        if set(value.keys()) == {'_id','members'}:
            ...
            if not validate_rs(value['configserver']):
                raise TypeError("Invalid configserver defifnition in sharded config")
            else:
                ...
                for member in value['configserver']['members']:
                    if member['host'] not in nodes:
                        ...
                        raise TypeError("Duplicated node name in sharded config")
            for shard in value['shards']:
                if not validate_rs(shard):
                    raise TypeError("Invalid shard definition in sharded config")
                else:
                    if shard['_id'] not in ids:
                        ids.append(shard['_id'])
                    else:
                        raise TypeError("Duplicated replicaset id in sharded config")
        ...
        if self.layout == "replicaset":
            for host in self.config['members']:
                ...
                docker.from_env().containers.run(
                    ...
                    detach=True,
                    network='test',
                    environment=["PBM_MONGODB_URI=mongodb://pbm:pbmpass@127.0.0.1:27017","MONGODB_EXTRA_ARGS= --port 27017 --replSet " + self.config['_id'] + " --configsvr --keyFile /etc/keyfile"],
                    volumes=["fs:/backups"]
                )
                if "arbiterOnly" in host:
                    if host['arbiterOnly']:
            ...
            for shard in self.config['shards']:
                ...
                for host in shard['members']:
                    ...
                    docker.from_env().containers.run(
                        ...
                        detach=True,
                        network='test',
                        environment=["PBM_MONGODB_URI=mongodb://pbm:pbmpass@127.0.0.1:27017","MONGODB_EXTRA_ARGS= --port 27017 --replSet " + shard['_id'] + " --shardsvr --keyFile /etc/keyfile"],
                        volumes=["fs:/backups"]
                        )
                    if 'arbiterOnly' in host:
                        if host['arbiterOnly']:
            ...
            for host in self.config['configserver']['members']:
                ...
                docker.from_env().containers.run(
                    ...
                    detach=True,
                    network='test',
                    environment=["PBM_MONGODB_URI=mongodb://pbm:pbmpass@127.0.0.1:27017","MONGODB_EXTRA_ARGS= --port 27017 --replSet " + self.config['configserver']['_id'] + " --configsvr --keyFile /etc/keyfile"],
                    volumes=["fs:/backups"]
                    )
                if "arbiterOnly" in host:
                    if host['arbiterOnly']:
    ...
    def __setup_authorizations(self,replicasets):
        [self.__setup_authorization(rs['members'][0]['host']) for rs in replicasets]
        time.sleep(5)
