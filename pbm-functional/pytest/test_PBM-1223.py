...
def test_load_PBM_T272(start_cluster, cluster, backup_t):
                ...
                try:
                    with session.start_transaction():
                        ...
                        Cluster.log(
                            collection
                            + ": ("
                            + str(session.cluster_time["clusterTime"].time)
                            + " "
                            + str(session.cluster_time["clusterTime"].inc)
                            + ") "
                            + str(j)
                        )
                        result.append(
                            (session.cluster_time["clusterTime"].time, session.cluster_time["clusterTime"].inc, j)
                        )
    ...
    pymongo.MongoClient("mongodb://root:root@rs101:27017").admin.command(
        {"setParameter": 1, "wiredTigerEngineRuntimeConfig": "cache_size=1G"}
    )
    pymongo.MongoClient("mongodb://root:root@rs201:27017").admin.command(
        {"setParameter": 1, "wiredTigerEngineRuntimeConfig": "cache_size=1G"}
    )
    cluster.check_pbm_status()
    upsert = True
    ...
    Cluster.log(
        "test1 documents count: " + str(pymongo.MongoClient(cluster.connection)["test"]["test1"].count_documents({}))
    )
    Cluster.log(
        "test2 documents count: " + str(pymongo.MongoClient(cluster.connection)["test"]["test2"].count_documents({}))
    )
    ...
    backup_meta = json.loads(
        testinfra.get_host("docker://rscfg01").check_output("cat /backups/" + backup + ".pbm.json")
    )
    ...
    def find_inserted(array_tuples, timestamp):
        ...
        print(timestamp)
        filtered_first = [
            t for t in array_tuples if (t[0] < timestamp[0]) or (t[0] == timestamp[0] and t[1] <= timestamp[1])
        ]
