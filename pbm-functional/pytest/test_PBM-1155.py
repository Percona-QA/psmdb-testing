...
def start_cluster(cluster, request):
    def insert_data(db, collection):
        ...
        while upsert:
            document = {"data": random.randbytes(1024)}
            try:
    ...
    try:
        ...
        for node in cluster.pbm_hosts:
            ...
            result = n.check_output(
                'mongo -u root -p root --eval \'db.getSiblingDB("admin").adminCommand({ "setParameter": 1, "wiredTigerEngineRuntimeConfig": "checkpoint=(log_size=1, wait=1),debug_mode=(slow_checkpoint=true)"})\''
            )
        upsert = True
        t1 = threading.Thread(
            target=insert_data,
