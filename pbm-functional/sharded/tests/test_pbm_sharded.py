...
def restart_mongos(node):
    with node.sudo():
        hostname = node.check_output('hostname -s')
        node.check_output('systemctl restart mongos')
...
def load_data(node,port,count):
    ...
    node.check_output('mgodatagen --uri=mongodb://127.0.0.1:' + port + '/ -f /tmp/generated_config.json --batchsize 10')
    ...
    if BACKUP_TYPE == "physical":
        ...
        for i in range(TIMEOUT):
            ...
            if not pitr:
                ...
                print("pitr enabled")
                break
        assert check_pitr(primary_cfg,"27019")
        time.sleep(10)
    else:
...
def test_7_disable_pitr():
    ...
    for i in range(TIMEOUT):
        ...
        if pitr:
            ...
            print("pitr disabled")
            break
    assert not check_pitr(primary_cfg,"27019")
