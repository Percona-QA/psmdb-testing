...
testinfra_hosts = testinfra.utils.ansible_runner.AnsibleRunner(os.environ["MOLECULE_INVENTORY_FILE"]).get_hosts("mongo")

primary_cfg = testinfra.utils.ansible_runner.AnsibleRunner(os.environ["MOLECULE_INVENTORY_FILE"]).get_host(
    "primary-cfg"
)

secondary1_cfg = testinfra.utils.ansible_runner.AnsibleRunner(os.environ["MOLECULE_INVENTORY_FILE"]).get_host(
    "secondary1-cfg"
)

secondary2_cfg = testinfra.utils.ansible_runner.AnsibleRunner(os.environ["MOLECULE_INVENTORY_FILE"]).get_host(
    "secondary2-cfg"
)

primary_rs0 = testinfra.utils.ansible_runner.AnsibleRunner(os.environ["MOLECULE_INVENTORY_FILE"]).get_host(
    "primary-rs0"
)

secondary1_rs0 = testinfra.utils.ansible_runner.AnsibleRunner(os.environ["MOLECULE_INVENTORY_FILE"]).get_host(
    "secondary1-rs0"
)

secondary2_rs0 = testinfra.utils.ansible_runner.AnsibleRunner(os.environ["MOLECULE_INVENTORY_FILE"]).get_host(
    "secondary2-rs0"
)

primary_rs1 = testinfra.utils.ansible_runner.AnsibleRunner(os.environ["MOLECULE_INVENTORY_FILE"]).get_host(
    "primary-rs1"
)

secondary1_rs1 = testinfra.utils.ansible_runner.AnsibleRunner(os.environ["MOLECULE_INVENTORY_FILE"]).get_host(
    "secondary1-rs1"
)

secondary2_rs1 = testinfra.utils.ansible_runner.AnsibleRunner(os.environ["MOLECULE_INVENTORY_FILE"]).get_host(
    "secondary2-rs1"
)
...
def make_backup(node, port, type):
    for i in range(TIMEOUT):
        ...
        if not running:
            if type == "physical":
                start = node.check_output(
                    "pbm backup --mongodb-uri=mongodb://localhost:" + port + "/ --out=json --type=" + type
                )
            else:
...
def make_restore(node, port, name):
    for i in range(TIMEOUT):
        running = check_status(node, port)
        if not running:
            output = node.check_output(
                "pbm restore --mongodb-uri=mongodb://localhost:" + port + "/ " + name + " --wait"
            )
            print(output)
            break
        else:
...
def make_pitr_restore(node, port, name, timestamp):
    for i in range(TIMEOUT):
        ...
        if not running:
            output = node.check_output(
                "pbm restore --mongodb-uri=mongodb://localhost:"
                + port
                + "/ --time="
                + timestamp
                + " --base-snapshot="
...
def make_pitr_replay(node, port, start, end):
    for i in range(TIMEOUT):
        ...
        if not running:
            output = node.check_output(
                "pbm oplog-replay --mongodb-uri=mongodb://localhost:"
                + port
                + "/ --start="
                + start
                + " --end="
...
def load_data(node, port, count):
    config = [
        {
            ...
            "collection": "binary",
            "count": 1,
            "shardConfig": {"shardCollection": "test.binary", "key": {"_id": "hashed"}},
            "content": {"binary": {"type": "binary", "minLength": 1048576, "maxLength": 1048576}},
        }
    ]
    ...
    result = node.check_output(
        "mgodatagen --uri=mongodb://127.0.0.1:" + port + "/ -f /tmp/generated_config.json --batchsize 10"
    )


def check_count_data(node, port):
    result = node.check_output(
        "mongo mongodb://127.0.0.1:" + port + "/test --eval 'db.binary.countDocuments({})' --quiet | tail -1"
    )
...
def setup_pitr(node, port):
    result = node.check_output(
        "pbm config --mongodb-uri=mongodb://localhost:" + port + "/ --set pitr.enabled=true --out=json"
    )
...
def test_1_setup_storage():
    result = primary_cfg.check_output(
        "pbm config --mongodb-uri=mongodb://localhost:27019/ --file=/etc/pbm-agent-storage-"
        + STORAGE
        + ".conf --out=json"
    )
    store_out = json.loads(result)
    ...
    d = {
        "numDownloadWorkers": numDownloadWorkers,
        "maxDownloadBufferMb": maxDownloadBufferMb,
        "downloadChunkMb": downloadChunkMb,
    }
    for k, v in d.items():
        if int(v):
            result = primary_cfg.check_output(
                "pbm config --mongodb-uri=mongodb://localhost:27017/ --set restore." + k + "=" + v + " --out=json"
            )
            store_out = json.loads(result)
            print(store_out)
    time.sleep(10)
...
def test_4_setup_pitr():
    ...
    if BACKUP_TYPE == "physical":
        result = primary_cfg.check_output(
            "pbm config --mongodb-uri=mongodb://localhost:27019/ --set pitr.enabled=true --set pitr.oplogOnly=true --out=json"
        )
        for i in range(TIMEOUT):
        ...
        result = primary_cfg.check_output(
            "pbm config --mongodb-uri=mongodb://localhost:27019/ --set pitr.enabled=true --out=json"
        )
    store_out = json.loads(result)
    print(store_out)
...
def test_7_disable_pitr():
    if EXISTING_BACKUP != "no" or CHECK_PITR == "no":
        pytest.skip("Skipping pitr test")
    result = primary_cfg.check_output(
        "pbm config --mongodb-uri=mongodb://localhost:27019/ --set pitr.enabled=false --out=json"
    )
    store_out = json.loads(result)
    print(store_out)
    for i in range(TIMEOUT):
