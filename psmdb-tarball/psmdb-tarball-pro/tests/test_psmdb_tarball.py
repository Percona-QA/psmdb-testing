...
def test_jstests(host, jstest):
    cmd = (
        "cd /percona-server-mongodb && /opt/venv/bin/python buildscripts/resmoke.py run --suite no_server /package-testing/jstests/"
        + jstest
    )
    with host.sudo():
        result = host.run(cmd)
...
def test_suites(host, suites):
    if host.system_info.distribution == "debian" or (
        host.system_info.distribution == "ubuntu" and "24.04" in host.system_info.release
    ):
