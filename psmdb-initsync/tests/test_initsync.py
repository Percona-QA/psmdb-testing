...
DOC_SIZE = int(os.getenv("OBJ_SIZE",1024))
...
def wait_secondary(node):
    wait = True
    while wait:
        state = node.run("timeout 1 mongosh --quiet --eval 'db.hello().secondary'")
...
def load_data(doc_size,idx_count,doc_count,db_count):
    ...
    primary.run_test('mgodatagen --uri=mongodb://127.0.0.1:27017/ -f /tmp/generated_config.json -b 10')
