...
PLM_VER = os.environ.get("PLM_VERSION")
...
def get_package_tuples():
    ...
    for software_files in SOFTWARE_FILES:
        data = 'version_files=percona-link-mongodb-' + PLM_VER + '|percona-link-mongodb&software_files=' + software_files
        ...
        if software_files == 'binary':
            assert "percona-link-mongodb-" + PLM_VER  in req.text
        else:
            assert "percona-link-mongodb-" + PLM_VER in req.text or "percona-link-mongodb_" + PLM_VER in req.text
...
def test_packages_site(software_files,filename,link):
    ...
    if not re.search(r'percona-link-mongodb.*\.diff\.gz', link):
