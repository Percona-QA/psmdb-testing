...
def get_package_tuples():
    ...
    for software_files in SOFTWARE_FILES:
        data = (
            "version_files=percona-distribution-mongodb-"
            + PDMDB_VER
            + "|percona-distribution-mongodb-"
            + FCV_VER
            + "&software_files="
            + software_files
        )
        ...
        if software_files == "binary":
            ...
            if (
                (PDMDB_VER.startswith("5") and version.parse(PDMDB_VER) > version.parse("5.0.27"))
                or (PDMDB_VER.startswith("6") and version.parse(PDMDB_VER) > version.parse("6.0.15"))
                or (PDMDB_VER.startswith("7") and version.parse(PDMDB_VER) > version.parse("7.0.12"))
            ):
                replacement_map = {"redhat/9": "ol9", "redhat/8": "ol8", "redhat/7": "ol7", "redhat/2023": "ol2023"}
                tar_os = [
                    replacement_map[os] if os in replacement_map else os
                    for os in SOFTWARE_FILES
                    if os not in ["source", "binary"]
                ]
                for os in tar_os:
                    assert "percona-server-mongodb-" + PSMDB_VER + "-x86_64." + os + "-minimal.tar.gz" in req.text
                    assert (
                        "percona-server-mongodb-" + PSMDB_VER + "-x86_64." + os + "-minimal.tar.gz.sha256sum"
                        in req.text
                    )
            ...
            assert (
                "percona-server-mongodb-" + PSMDB_VER + "." in req.text
                or "percona-server-mongodb_" + PSMDB_VER + "." in req.text
            )
            assert (
                "percona-server-mongodb-server-" + PSMDB_VER in req.text
                or "percona-server-mongodb-server_" + PSMDB_VER in req.text
            )
            assert (
                "percona-server-mongodb-mongos-" + PSMDB_VER in req.text
                or "percona-server-mongodb-mongos_" + PSMDB_VER in req.text
            )
            assert (
                "percona-server-mongodb-tools-" + PSMDB_VER in req.text
                or "percona-server-mongodb-tools_" + PSMDB_VER in req.text
            )
            ...
            if version.parse(PSMDB_VER) > version.parse("6.0.0"):
                ...
                assert (
                    "percona-server-mongodb-shell-" + PSMDB_VER in req.text
                    or "percona-server-mongodb-shell_" + PSMDB_VER in req.text
                )
...
def test_packages_site(software_files, filename, link):
    ...
    if not re.search(r"percona-telemetry-agent.*\.diff\.gz", link) and not re.search(
        r"percona-backup-mongodb.*\.diff\.gz", link
    ):
