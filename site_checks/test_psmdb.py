...
if version.parse(PSMDB_VER) > version.parse("8.0.0"):
    SOFTWARE_FILES = ["bookworm", "binary", "redhat/9", "redhat/8", "source", "jammy", "noble", "redhat/2023"]
...
def get_package_tuples():
    ...
    for software_files in SOFTWARE_FILES:
        data = (
            "version_files=percona-server-mongodb-"
            + PSMDB_VER
        ...
        if software_files == "binary":
            if (
                (MAJ_VER.startswith("5") and version.parse(MAJ_VER) > version.parse("5.0.27"))
                or (MAJ_VER.startswith("6") and version.parse(MAJ_VER) > version.parse("6.0.15"))
                or (MAJ_VER.startswith("7") and version.parse(MAJ_VER) > version.parse("7.0.12"))
                or (MAJ_VER.startswith("8") and version.parse(MAJ_VER) > version.parse("8.0.0"))
            ):
