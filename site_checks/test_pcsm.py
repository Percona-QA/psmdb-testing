...
def get_package_tuples():
    ...
    for software_files in SOFTWARE_FILES:
        data = 'version_files=percona-clustersync-mongodb-' + PCSM_VER + '|percona-clustersync-mongodb&software_files=' + software_files
        ...
        if software_files == 'binary':
            assert "percona-clustersync-mongodb-" + PCSM_VER  in req.text
        else:
            assert "percona-clustersync-mongodb-" + PCSM_VER in req.text or "percona-clustersync-mongodb_" + PCSM_VER in req.text
