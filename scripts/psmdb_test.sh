...
if [ -z "$1" ]; then
  echo "This script needs parameter 3.0|3.2|3.4|3.6|4.0|4.2|4.4|5.0|6.0"
  exit 1
elif [ "$1" != "3.0" -a "$1" != "3.2" -a "$1" != "3.4" -a "$1" != "3.6" -a "$1" != "4.0" -a "$1" != "4.2" -a "$1" != "4.4" -a "$1" != "5.0" -a "$1" != "6.0" ]; then
...
for engine in mmapv1 PerconaFT rocksdb inMemory wiredTiger; do
  ...
  elif [ "$1" = "4.0" -o "$1" = "4.2" -o "$1" = "4.4" -o "$1" = "5.0" -o "$1" = "6.0" ] && [ "${engine}" == "rocksdb" ]; then
    echo "Skipping rocksdb because version is >3.6"
  elif [ "$1" = "4.2" -o "$1" = "4.4" -o "$1" = "5.0" -o "$1" = "6.0" ] && [ "${engine}" == "mmapv1" ]; then
