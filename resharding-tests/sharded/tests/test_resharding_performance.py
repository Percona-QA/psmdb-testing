    ...
    config = [
        {
            ...
            "collection": "binary",
            "count": 1,
            "shardConfig": {"shardCollection": "test.binary", "key": {"num": "hashed"}},
            "content": {
            ...
            "indexes": [{"name": "idx_1", "key": {"str": "hashed"}}],
        }
    ]
...
def append_data(timeout):
    ...
    print(config_json)
    primary_cfg.run_test("echo '" + config_json + "' > /tmp/generated_config.json")
    primary_cfg.run_test(
        "timeout -s 9 "
        + timeout
        + " mgodatagen --uri=mongodb://127.0.0.1:27017/ -f /tmp/generated_config.json -n 1 -a -b 100 >/tmp/append.txt 2>&1 &"
    )
...
def check_count_data():
    result = primary_cfg.check_output(
        "mongo mongodb://127.0.0.1:27017/test --eval 'db.binary.countDocuments({})' --quiet | tail -1"
    )
...
def check_distribution_info():
    result = primary_cfg.check_output(
        "mongo mongodb://127.0.0.1:27017/test --eval 'db.binary.getShardDistribution()' --quiet"
    )
...
def reshard_collection():
    result = primary_cfg.check_output(
        'mongo mongodb://127.0.0.1:27017/admin --eval \'db.runCommand({reshardCollection: "test.binary", key: {"str": "hashed"}})\' --quiet'
    )
