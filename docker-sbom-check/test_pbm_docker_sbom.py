...
IMAGE_REPO = os.environ.get("IMAGE_REPO", "percona")
PBM_IMAGE = f"docker.io/{IMAGE_REPO}/percona-backup-mongodb:{PBM_VER}"


def test_pbm_docker_embedded_sbom():
    ...
    sbom_path_in_image = f"/usr/share/doc/percona-backup-mongodb/{sbom_filename}"

    result = subprocess.run(
        ["docker", "run", "--rm", PBM_IMAGE, "cat", sbom_path_in_image],
        capture_output=True,
    )
...
def test_pbm_docker_oci_sbom():
    ...
    inspect_result = subprocess.run(
        ["docker", "manifest", "inspect", PBM_IMAGE],
        capture_output=True, text=True,
    )
    ...
    image_base = PBM_IMAGE.split(":")[0]
    amd64_ref = f"{image_base}@{amd64_digest}"

    trivy_result = subprocess.run(
        ["trivy", "image", "--severity", "HIGH,CRITICAL", "--ignore-unfixed",
         "--exit-code", "1", "--sbom-sources", "oci", PBM_IMAGE],
        capture_output=True, text=True,
    )
    assert trivy_result.returncode == 0, (
