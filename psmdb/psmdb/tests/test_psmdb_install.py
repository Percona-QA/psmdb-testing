...
def check_hotbackup(node):
    node.check_output(
        "mongo --quiet --eval \"for(i=1; i <= 1000; i++) { db.series.insert( { id: i, name: 'series'+i })}\""
    )
...
def test_telemetry(host):
    ...
    with host.sudo():
        ...
        if host.system_info.distribution == "debian" or host.system_info.distribution == "ubuntu":
            host.check_output(
                "sed -E 's|PERCONA_TELEMETRY_URL=(.+)|PERCONA_TELEMETRY_URL=https://check-dev.percona.com/v1/telemetry/GenericReport|' -i /etc/default/percona-telemetry-agent"
            )
            host.check_output(
                "sed -E 's|PERCONA_TELEMETRY_CHECK_INTERVAL=(.+)|PERCONA_TELEMETRY_CHECK_INTERVAL=10|' -i /etc/default/percona-telemetry-agent"
            )
        else:
            host.check_output(
                "sed -E 's|PERCONA_TELEMETRY_URL=(.+)|PERCONA_TELEMETRY_URL=https://check-dev.percona.com/v1/telemetry/GenericReport|' -i /etc/sysconfig/percona-telemetry-agent"
            )
            host.check_output(
                "sed -E 's|PERCONA_TELEMETRY_CHECK_INTERVAL=(.+)|PERCONA_TELEMETRY_CHECK_INTERVAL=10|' -i /etc/sysconfig/percona-telemetry-agent"
            )
...
def test_auth(host, auth):
    ...
    if auth == "LDAP":
        ...
        result = host.check_output(
            'mongo admin --quiet --eval \'db.createRole({role: "cn=testwriters,ou=groups,dc=percona,dc=com", privileges: [], roles: [ "userAdminAnyDatabase", "clusterMonitor", "clusterManager", "clusterAdmin"]})\''
        )
        print(result)
    if auth == "GSSAPI":
        conf["setParameter"]["authenticationMechanisms"] = "GSSAPI"
        result = host.check_output(
            'mongo admin --quiet --eval \'db.getSiblingDB("$external").runCommand({createUser:"exttestrw@PERCONATEST.COM",roles: [{role: "userAdminAnyDatabase", db: "admin"}]})\''
        )
        print(result)
    if auth == "MONGODB-AWS":
        conf["setParameter"]["authenticationMechanisms"] = "MONGODB-AWS"
        result = host.check_output(
            'mongo admin --quiet --eval \'db.getSiblingDB("$external").runCommand({createUser:"arn:aws:iam::119175775298:role/jenkins-psmdb-slave",roles: [{role: "userAdminAnyDatabase", db: "admin"}]})\''
        )
        print(result)
    ...
    if auth == "LDAP":
        result = host.check_output(
            'mongo -u "cn=exttestrw,ou=people,dc=percona,dc=com" -p "exttestrw9a5S" --authenticationDatabase \'$external\' --authenticationMechanism=PLAIN --quiet --eval "db.runCommand({connectionStatus : 1})"'
        )
        print(result)
    ...
    if auth == "GSSAPI":
        with host.sudo():
            ...
            result = host.check_output(
                "mongo -u exttestrw@PERCONATEST.COM --host "
                + hostname
                + " --authenticationMechanism=GSSAPI --authenticationDatabase '$external' --quiet --eval \"db.runCommand({connectionStatus : 1})\""
            )
            print(result)
            assert "ok: 1" in result or '"ok" : 1' in result
    if auth == "MONGODB-AWS":
        ...
        result = host.check_output(
            "mongo --host "
            + hostname
            + " --authenticationMechanism MONGODB-AWS --authenticationDatabase '$external' --quiet --eval \"db.runCommand({connectionStatus : 1})\""
        )
        print(result)
        assert "ok: 1" in result or '"ok" : 1' in result
